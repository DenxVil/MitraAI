...
class MitraTelegramBot:
    ...
    def __init__(self):
        ...
        self.rate_limiter = RateLimiter(
            max_requests=settings.rate_limit_messages_per_minute, window_seconds=60
        )
...
Keep chatting! 💬"""
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        logger.info("message_received", user_id=user_id, message_length=len(message_text))
        ...
        try:
            ...
            response = await self.engine.process_message(user_id=user_id, message=message_text)

            # Send response
            await update.message.reply_text(response)

            logger.info(
                "message_handled_successfully", user_id=user_id, response_length=len(response)
            )
    ...
    def _get_or_create_user(self, telegram_user) -> User:
        ...
        logger.info("user_created", user_id=user_id, username=telegram_user.username)
